...
--CREATE WAREHOUSE dbt_wh WAREHOUSE_SIZE = SMALL;
...
--CREATE SCHEMA IF NOT EXISTS DBT_ANALYTICS.dev;
--CREATE SCHEMA IF NOT EXISTS DBT_ANALYTICS.prod;
-- Used for storing objects Snowflake needs for GitHub integration (secrets, etc.)
--CREATE SCHEMA IF NOT EXISTS DBT_ANALYTICS.integrations;
-- Used for the Tasty Bytes foundational source data loaded from S3
--CREATE SCHEMA IF NOT EXISTS DBT_ANALYTICS.raw;
...
ALTER SCHEMA DBT_ANALYTICS.dev SET LOG_LEVEL = 'INFO';
ALTER SCHEMA DBT_ANALYTICS.dev SET TRACE_LEVEL = 'ALWAYS';
ALTER SCHEMA DBT_ANALYTICS.dev SET METRIC_LEVEL = 'ALL';

ALTER SCHEMA DBT_ANALYTICS.prod SET LOG_LEVEL = 'INFO';
ALTER SCHEMA DBT_ANALYTICS.prod SET TRACE_LEVEL = 'ALWAYS';
ALTER SCHEMA DBT_ANALYTICS.prod SET METRIC_LEVEL = 'ALL';
...
USE DBT_ANALYTICS.integrations;
CREATE OR REPLACE SECRET DBT_ANALYTICS.integrations.tb_dbt_git_secret
  TYPE = password
...
CREATE OR REPLACE API INTEGRATION tb_dbt_git_api_integration
  ...
  ALLOWED_AUTHENTICATION_SECRETS = (DBT_ANALYTICS.integrations.tb_dbt_git_secret)
  ENABLED = TRUE;
...
CREATE OR REPLACE FILE FORMAT DBT_ANALYTICS.public.csv_ff 
type = 'csv';

CREATE OR REPLACE STAGE DBT_ANALYTICS.public.s3load
COMMENT = 'Quickstarts S3 Stage Connection'
url = 's3://sfquickstarts/frostbyte_tastybytes/'
file_format = DBT_ANALYTICS.public.csv_ff;
...
-- country table build
CREATE OR REPLACE TABLE DBT_ANALYTICS.raw.country
(
...
CREATE OR REPLACE TABLE DBT_ANALYTICS.raw.franchise 
(
...
CREATE OR REPLACE TABLE DBT_ANALYTICS.raw.location
(
...
-- menu table build
CREATE OR REPLACE TABLE DBT_ANALYTICS.raw.menu
(
    menu_id NUMBER(19,0),
...
-- truck table build
CREATE OR REPLACE TABLE DBT_ANALYTICS.raw.truck
(
...
CREATE OR REPLACE TABLE DBT_ANALYTICS.raw.order_header
(
...
CREATE OR REPLACE TABLE DBT_ANALYTICS.raw.order_detail 
(
...
CREATE OR REPLACE TABLE DBT_ANALYTICS.raw.customer_loyalty
(
    customer_id NUMBER(38,0),
    first_name VARCHAR(16777216),
    last_name VARCHAR(16777216),
...
-- country table load
COPY INTO DBT_ANALYTICS.raw.country
FROM @DBT_ANALYTICS.public.s3load/raw_pos/country/;

-- franchise table load
COPY INTO DBT_ANALYTICS.raw.franchise
FROM @DBT_ANALYTICS.public.s3load/raw_pos/franchise/;

-- location table load
COPY INTO DBT_ANALYTICS.raw.location
FROM @DBT_ANALYTICS.public.s3load/raw_pos/location/;

-- menu table load
COPY INTO DBT_ANALYTICS.raw.menu
FROM @DBT_ANALYTICS.public.s3load/raw_pos/menu/;

-- truck table load
COPY INTO DBT_ANALYTICS.raw.truck
FROM @DBT_ANALYTICS.public.s3load/raw_pos/truck/;

-- customer_loyalty table load
COPY INTO DBT_ANALYTICS.raw.customer_loyalty
FROM @DBT_ANALYTICS.public.s3load/raw_customer/customer_loyalty/;

-- order_header table load
COPY INTO DBT_ANALYTICS.raw.order_header
FROM @DBT_ANALYTICS.public.s3load/raw_pos/order_header/;

-- order_detail table load
COPY INTO DBT_ANALYTICS.raw.order_detail
FROM @DBT_ANALYTICS.public.s3load/raw_pos/order_detail/;

-- =============================================================================
-- Setup complete
-- =============================================================================

SELECT 'DBT_ANALYTICS setup is now complete' AS note;
